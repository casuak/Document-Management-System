let app = new Vue({
    ...
    data: {
        urls: {
            // api for entity
            insertEntity: '/api/sys/dictType/insert',
            deleteEntityListByIds: '/api/sys/dictType/deleteListByIds',
            updateEntity: '/api/sys/dictType/update',
            selectEntityListByPage: '/api/sys/dictType/selectListByPage',
            selectEntityAllList: '/api/sys/dictType/selectAllList'
        },
            ...
            this.$refs['form_insertEntity'].validate((valid) => {
                if (valid) {
                    ...
                    ajaxPostJSON(app.urls.insertEntity, data, function (d) {
                        ...
                        window.parent.parent.app.showMessage('添加成功！', 'success');
                        app.refreshTable_entity(); // 添加完成后刷新页面
                    }, function () {
                        app.dialog.insertEntity.loading = false;
                        app.dialog.insertEntity.visible = false;
                        window.parent.parent.app.showMessage('添加失败！', 'error');
                    });
                } else {
                    console.log("表单数据不合法！");
                    return false;
            ...
            this.$refs['form_updateEntity'].validate((valid) => {
                if (valid) {
                    ...
                    ajaxPostJSON(app.urls.updateEntity, data, function (d) {
                        ...
                        window.parent.parent.app.showMessage('编辑成功！', 'success');
                        app.refreshTable_entity(); // 编辑完成后刷新页面
                    }, function () {
                        app.dialog.updateEntity.loading = false;
                        app.dialog.updateEntity.visible = false;
                        window.parent.parent.app.showMessage('编辑失败！', 'error');
                    });
                } else {
                    console.log("表单数据不合法！");
                    return false;
