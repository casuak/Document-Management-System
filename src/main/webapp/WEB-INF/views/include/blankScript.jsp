<%@ page contentType="text/html;charset=UTF-8" %>

<script src="/static/plugins/jquery/jquery-3.3.1.min.js"></script>
<script src="/static/plugins/axios@0.18.0/dist/axios.min.js"></script>

<script src="/static/plugins/vue/vue.min.js"></script>

<script src="/static/plugins/iview@3.3.3/dist/iview.min.js"></script>
<%--<script src="//unpkg.com/iview/dist/iview.min.js"></script>--%>

<script src="/static/plugins/bootstrap-3.3.7-dist/js/bootstrap.min.js"></script>

<script src="/static/plugins/element-ui@2.7.0/lib/index.js"></script>

<%-- upload file --%>
<script src="/static/plugins/crypto1/crypto/crypto.js"></script>
<script src="/static/plugins/crypto1/hmac/hmac.js"></script>
<script src="/static/plugins/crypto1/sha1/sha1.js"></script>
<script src="/static/plugins/plupload-2.1.2/base64.js"></script>
<script src="/static/plugins/plupload-2.1.2/js/plupload.full.min.js"></script>

<%-- 自定义常用全局函数、常量 --%>
<script src="/static/utils/js/common.js"></script>
<script src="/static/utils/js/download.js"></script>
